...
{
	auto* volatile pLeftByte = static_cast<const volatile std::uint8_t*>(pLeftBuffer);
	auto* volatile pRightByte = static_cast<const volatile std::uint8_t*>(pRightBuffer);

	volatile std::uint8_t uResult = 0U;
	for (std::size_t i = 0U; i < nCount; ++i)
		uResult |= pLeftByte[i] ^ pRightByte[i];
